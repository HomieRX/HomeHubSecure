...
export interface IStorage {
  ...
  // Manager scheduling controls
  upsertManagerSettings(settings: {
    homeManagerId: string;
    maxDailyJobs: number;
    serviceWindowStart: string;
    serviceWindowEnd: string;
  ...
  getManagerSettings(homeManagerId: string): Promise<ManagerSettings | undefined>;
  createManagerTimeBlock(block: {
    homeManagerId: string;
    blockType: "BLACKOUT" | "TIME_OFF" | "TRAINING" | "MAINTENANCE";
    startAt: Date;
    endAt: Date;
  ...
  getManagerTimeBlocks(homeManagerId: string): Promise<ManagerTimeBlock[]>;
...
export class MemStorage implements IStorage {
  ...
  private managerSettings: Map<string, ManagerSettings>;
  private managerTimeBlocks: Map<string, ManagerTimeBlock>;
  ...
  constructor() {
    ...
    this.managerSettings = new Map();
    this.managerTimeBlocks = new Map();
  }
  ...
  async getManagerSettings(homeManagerId: string): Promise<ManagerSettings | undefined> {
    return this.managerSettings.get(homeManagerId);
  }

  async upsertManagerSettings(settings: {
    homeManagerId: string;
    maxDailyJobs: number;
    serviceWindowStart: string;
    serviceWindowEnd: string;
    ...
    const now = new Date();
    const existing = this.managerSettings.get(settings.homeManagerId);
    const record: ManagerSettings = existing
      ? {
          ...existing,
          maxDailyJobs: settings.maxDailyJobs,
          serviceWindowStart: settings.serviceWindowStart,
          serviceWindowEnd: settings.serviceWindowEnd,
          bufferMinutes: settings.bufferMinutes,
          updatedAt: now,
        }
      : {
          homeManagerId: settings.homeManagerId,
          maxDailyJobs: settings.maxDailyJobs,
          serviceWindowStart: settings.serviceWindowStart,
          serviceWindowEnd: settings.serviceWindowEnd,
          bufferMinutes: settings.bufferMinutes,
    ...
    this.managerSettings.set(settings.homeManagerId, record);
    return record;
  }

  async createManagerTimeBlock(block: {
    homeManagerId: string;
    blockType: "BLACKOUT" | "TIME_OFF" | "TRAINING" | "MAINTENANCE";
    startAt: Date;
    endAt: Date;
    ...
    this.managerTimeBlocks.set(id, record);
    return record;
  }

  async getManagerTimeBlocks(homeManagerId: string): Promise<ManagerTimeBlock[]> {
    return Array.from(this.managerTimeBlocks.values()).filter(block => block.homeManagerId === homeManagerId);
  }
